...
PACKAGE_BASENAME="linkloom-firefox-${VERSION}"
PACKAGE_PATH="$DIST_DIR/${PACKAGE_BASENAME}.xpi"
...
if command -v sha256sum >/dev/null 2>&1; then
  (cd "$DIST_DIR" && sha256sum "${PACKAGE_BASENAME}.xpi" > "${PACKAGE_BASENAME}.sha256")
elif command -v shasum >/dev/null 2>&1; then
  (cd "$DIST_DIR" && shasum -a 256 "${PACKAGE_BASENAME}.xpi" > "${PACKAGE_BASENAME}.sha256")
fi
